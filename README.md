...
5. Create a A record in your DNS zone for the domain name. The wildcard domain should point to the IP address of the gateway.
   ...
   ;; ANSWER SECTION:
   test-gateway.microserve.dev. 3600 IN    A       35.190.63.142
   [...]
   ```
...
Create a database for the application. We'll name it `cms`.
...
Give the service account the instance User role so it can access the instance.
...
Give the service account the Cloud SQL Client role so it can perform SQL queries.
...
And create a SQL User the service account will use to connect to the database.
...
HTTP_ROUTE_HOSTNAME=staging.$DOMAIN_NAME
EOF
```
...
TRANSFER_TOKEN_SALT=$(echo -n "TRANSFER_TOKEN_SALTstaging" | md5sum | cut -d ' ' -f 1)
JWT_SECRET=$(echo -n "JWT_SECRETstaging" | md5sum | cut -d ' ' -f 1)
DATABASE_URL=postgres://$CMS_SERVICE_ACCT%40$PROJECT_ID.iam:@localhost:5432/cms-staging
EOF
```

5. Create a file `frontend/kubernetes-manifests/prod/.env.config`
...
6. Create a file `frontend/kubernetes-manifests/prod/.env.secret`
...
4. Change the `DATABASE_URL` value in `.env.secret` with your `cms-$SKAFFOLD_NAMESPACE`.

5. Create a `.env.config` file with the following values

```bash
cat << EOF > cms/kubernetes-manifests/qa/.env.config
PROJECT_ID=mgalite-ts24-playground
...
6. Create a `.env.config` file with the following values
...
7. Create a `.env.secret` file with the following values
